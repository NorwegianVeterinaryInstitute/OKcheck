...
ktr <- PJSdata %>%
  dplyr::select(saksnr, aar, ansvarlig_seksjon, innsendelsenr, provenr, konkl_type, konkl_kjennelsekode, konkl_analyttkode, avsluttet) %>%
  dplyr::distinct() %>%
  dplyr::filter(konkl_type == 3 & !is.na(avsluttet)) %>%
