...
if (length(code_2_accept) > 0) {
  ...
  for (k in 1:length(code_variable)) {
    index <- union(index,
                   NVIpjsr::PJS_levels[which(NVIpjsr::PJS_levels[1:8, which(NVIpjsr::PJS_levels[which(NVIpjsr::PJS_levels$variable == code_variable[k]), ] == 1)[1]] == 1), "variable"])
  }
  ...
  for (j in c("hensiktkode", code_variable)) {
    if (j %in% intersect(NVIpjsr::PJS_code_description_colname$code_colname, colnames(ktr))) {
      ktr <- NVIpjsr::add_PJS_code_description(data = ktr,
